...
```text
course-root/
├── README.md                          # 课程根说明：目录结构、使用顺序、学习闭环
├── course-rules.md                    # 课程规则层：身份、允许源、禁止源、出题边界、教学读取顺序
├── agent-persona.md                   # AI 教学人格：姿态、偏好、禁止姿态
├── mastery-loop.md                    # Two Sigma 掌握循环：10 步闭环定义、掌握标准、个性化规则
│
├── textbook/                          # 教材层（只读，不被修改）
│   ├── index.md                       # 教材索引：文件导航、段落号路由表、使用规则
│   ├── chapters/                      # 每章一个 Markdown 文件，正文用 ¶XXXX 段落号标注
│   │   ├── 00-序前言符号表.md
│   │   ├── 01-第1章-绪论.md
│   │   ├── ...
│   │   └── img/                       # 教材图片
│   └── pdf/                           # 教材 PDF 原文（Markdown 疑似错位时核对）
│
├── knowledge/                         # 教学控制层（教材之上的元数据）
│   ├── README.md
│   ├── teaching-guides/               # 每个小节一个 .teaching.md 教学指引
│   │   ├── README.md
│   │   ├── TEMPLATE.md                # 教学指引模板
│   │   ├── chapter-01/                # 按章分目录
...
│   │   ├── chapter-02/
│   │   └── ...
│   └── coverage/                      # 知识点覆盖审计表（每章一个）
│       ├── README.md
│       ├── chapter-01-coverage.md
│       └── ...
│
├── learning-path/                     # 课程调度路线
│   ├── course-map.md                  # 章级导航总表
│   ├── chapter-01.md                  # 每章详细小节路线（单元ID、知识点指引、教材段落、状态）
│   ├── chapter-02.md
│   └── ...
│
├── progress/                          # 学习进度层
│   ├── README.md                      # 进度说明：掌握度等级、状态含义、推进规则
│   ├── current-position.md            # 当前学习入口（AI 每次先读这个）
│   └── mastery-tracker.md             # 掌握度追踪表（每单元一行，含等级、正确率、弱点、推进）
│
├── practice/                          # 练习层
│   ├── README.md
│   ├── task-generation-rules.md       # 出题规则：源隔离、三层标注、允许/禁止题型、难度控制
│   ├── daily-diagnostics.md           # 诊断短测模板（每节循环的 6 步模板）
│   ├── task-bank.md                   # 可复用训练任务库
│   ├── chapter-01-unit-spec.md        # 每章单元练习规格（允许题型、来源边界、未学内容拦截）
│   └── ...
│
├── review/                            # 复盘层
│   ├── README.md
│   └── mistakes.md                    # 错题与误区主表（含纠正任务和复测状态）
│
└── logs/                              # 日志层
    └── learning-sessions/             # 每次学习闭环一个日志文件
        ├── README.md                  # 日志模板和验收清单
...
| 2 | 明确学习目标 | learning-path/、coverage/、teaching-guides/ | 本节目标、教材段落、掌握边界 |
...
- 标题行不占用段落号
- 所有引用（教学指引、覆盖审计、练习规格、学习日志）都指向段落号
...
#### 3.5.2 覆盖审计表（coverage/）

**每章一个 `chapter-XX-coverage.md`**，用来确认学习路线、教学指引、教材段落、练习边界和掌握追踪之间没有孤岛。
...
- 正式题范围只包含 `出题权限=正式题` 的核心知识点
- `课堂识别不计分` 只能轻量确认，不计入正确率
- 新增或调整学习路线时必须同步更新

**复刻要点**：
- 这是确保系统一致性的关键文件
- 每章建完学习路线和教学指引后必须做一次覆盖审计
...
- 单元ID 必须全局唯一，且在 coverage、mastery-tracker、learning-sessions 中保持一致
...
**文件**：
- `task-generation-rules.md`：出题的完整规则集
- `daily-diagnostics.md`：每节循环的诊断短测模板
- `task-bank.md`：可复用训练任务库
- `chapter-XX-unit-spec.md`：每章单元练习规格
...
- `chapter-XX-unit-spec.md` 需要为每章新建
- `task-bank.md` 随学习过程积累
...
learning-path/chapter-XX.md ─── 小节路线 ────────────────────────┤
        │                                                         │
        ▼                                                         │
knowledge/coverage/chapter-XX-coverage.md ─── 覆盖审计 ──────────┤
        │                                                         │
        ▼                                                         │
...
practice/chapter-XX-unit-spec.md ─── 单元练习规格 ───────────────┤
practice/daily-diagnostics.md ─── 诊断短测模板 ──────────────────┤
                                                                  │
...
2. 查 `coverage/chapter-XX-coverage.md` 确认连通性
3. 查 `teaching-guides/.../*.teaching.md` 确认知识点分层
4. 按指引读 `textbook/chapters/*.md` 的指定段落
5. 按 `practice/` 规则生成练习
6. 更新 `progress/`、`review/`、`logs/`
...
│  - 读 practice/chapter-XX-unit-spec.md                        │
│  - 生成 3-5 道带五行标注的练习题                              │
└──────────────────────────┬──────────────────────────────────┘
                           ▼
┌──────────────────────────────────────────────────────────────┐
...
**暂不生成的文件**（需要逐章手动或 AI 辅助编写）：
- `knowledge/teaching-guides/chapter-XX/*.teaching.md`
- `knowledge/coverage/chapter-XX-coverage.md`
- `learning-path/chapter-XX.md`
- `practice/chapter-XX-unit-spec.md`
...
流水线完成后，`textbook/` 和课程骨架目录已就绪，进入后续步骤。
...
1. 为每章编写 `knowledge/coverage/chapter-XX-coverage.md`
2. 确认每个单元都有：教学指引 → 教材段落 → 正式题范围 → 掌握追踪行 → 下一单元
3. 确认没有孤岛
...
3. 复制 `practice/task-bank.md`（初始为空，随学习积累）
4. 为每章编写 `practice/chapter-XX-unit-spec.md`（单元练习规格表）
...
| `knowledge/coverage/` | 100% | 每章的覆盖审计表 |
| `learning-path/` | 100% | 课程路线和单元划分 |
| `practice/chapter-XX-unit-spec.md` | 100% | 每章单元练习规格 |
